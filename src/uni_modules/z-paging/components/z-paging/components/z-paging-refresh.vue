    ...
    >
      <view class="zp-r-left">
        ...
        <image
          v-else
          :class="{
            'zp-line-loading-image': refreshingAnimated,
            'zp-r-left-image': true,
            'zp-r-left-image-pre-size-rpx': unit === 'rpx',
            'zp-r-left-image-pre-size-px': unit === 'px',
          }"
          :style="[leftImageStyle, imgStyle]"
          :src="leftImageSrc"
        />
        ...
        >
          <loading-indicator
            :class="
              isIos
                ? {
                    'zp-loading-image-ios-rpx': unit === 'rpx',
                    'zp-loading-image-ios-px': unit === 'px',
                  }
                : {
                    'zp-loading-image-android-rpx': unit === 'rpx',
                    'zp-loading-image-android-px': unit === 'px',
                  }
            "
            :style="[{ color: zTheme.indicator[ts] }, imgStyle]"
            :animating="true"
          />
        </view>
        <!-- #endif -->
      ...
      <view class="zp-r-right">
        ...
        <!-- 右侧下拉刷新时间文字 -->
        <text
          v-if="showUpdateTime && refresherTimeText.length"
          class="zp-r-right-text"
          :class="{
            'zp-r-right-time-text-rpx': unit === 'rpx',
            'zp-r-right-time-text-px': unit === 'px',
          }"
          :style="[{ color: zTheme.title[ts] }, updateTimeStyle]"
        >
          {{ refresherTimeText }}
        </text>
      </view>
...
export default {
  ...
  data() {
    return {
      ...
      zTheme: {
        ...
        success: {
          white: zStatic.base64SuccessWhite,
          black: zStatic.base64Success,
        },
  ...
  computed: {
    ...
    leftImageStyle() {
      ...
      const size = showUpdateTime
        ? u.addUnit(36, this.unit)
        : u.addUnit(34, this.unit);
      return {
        width: size,
        height: size,
        "margin-right": showUpdateTime
          ? u.addUnit(20, this.unit)
          : u.addUnit(9, this.unit),
      };
    },
    // 左侧图片src
    leftImageSrc() {
    ...
    rightTextStyle() {
      let stl = {};
      // #ifdef APP-NVUE
      const textHeight = this.showUpdateTime
        ? u.addUnit(40, this.unit)
        : u.addUnit(80, this.unit);
